...
//    }
    ...
    {
        assert(false);
        return std::vector<Car_HEAD_TAIL_Result>();
    }
    ...
    for (int t = 0; t < totaldata.size(); t++)
    {
        ...
        Car_HEAD_TAIL_Result re = Detect_CarHeadAndTail_GPU(iInstanceId, batchdata, copyednum); //获得检测结果
    ...
    std::vector<Car_HEAD_TAIL_Result> final_result(batchsize);
