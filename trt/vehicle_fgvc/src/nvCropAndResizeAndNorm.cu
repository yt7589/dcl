...
__device__ __forceinline__ unsigned char get_val(unsigned char  *data,int c, int y,int x,int W,int detx1,int detx2,int dety1,int dety2,
                                                 int pdetx1,int pdetx2,int pdety1,int pdety2, unsigned char pad) {

    if (x < detx1 || x >= detx2 || y <dety1 || y >= dety2 || (x >= pdetx1 && x < pdetx2 && y >= pdety1 && y < pdety2) ){
        return pad;
    }else{
#if __CUDA_ARCH__ >= 350
        return __ldg(&data[(y*W+x)*3+c]);
...
__forceinline__ __global__ void nvCropAndResizeAndNormKernel(unsigned char * Src, float * cropImage,ITS_Vehicle_Result_Detect* det,
            int srcWidth,int srcHeight,int outW,int outH,float3 means,float3 stds)
{
...
#pragma unroll
    for (int channel=0; channel < 3; channel++){
        ...
        *dst = (val /255.0f-(&means.x)[channel])/(&stds.x)[channel];

    }
}

__forceinline__ __global__ void nvCropAndResizeAndNormKernel(unsigned char * Src, float * cropImage,ITS_Vehicle_Result_Detect* det,
                                                             ITS_Vehicle_Result_Detect* Pdet,
                                                             int srcWidth,int srcHeight,int outW,int outH,float3 means,float3 stds)
{
    int x = threadIdx.x + blockIdx.x * blockDim.x;
    int y = threadIdx.y + blockIdx.y * blockDim.y;
    ...
    int pdetx1 = Pdet[0].iLeft[detIndex]  ,  pdety1 = Pdet[0].iTop[detIndex];
    int pdetx2 = Pdet[0].iRight[detIndex] ,  pdety2 = Pdet[0].iBottom[detIndex];
...
#pragma unroll
    for (int channel=0; channel < 3; channel++){
        // chw
        dst = cropImage + ((detIndex*3 + 2 - channel)*outH + y)*outW + x ;
        val = x0lambda*y0lambda* static_cast<float >(get_val(Src,channel,y1,x1,srcWidth,detx1,detx2,dety1,dety2,pdetx1,pdetx2,pdety1,pdety2,0)) +
              x0lambda*y1lambda* static_cast<float >(get_val(Src,channel,y1+y1p,x1,srcWidth,detx1,detx2,dety1,dety2,pdetx1,pdetx2,pdety1,pdety2,0)) +
              x1lambda*y0lambda* static_cast<float >(get_val(Src,channel,y1,x1+x1p,srcWidth,detx1,detx2,dety1,dety2,pdetx1,pdetx2,pdety1,pdety2,0)) +
              x1lambda*y1lambda* static_cast<float >(get_val(Src,channel,y1+y1p,x1+x1p,srcWidth,detx1,detx2,dety1,dety2,pdetx1,pdetx2,pdety1,pdety2,0)) ;
        *dst = (val/255.0f-(&means.x)[channel])/(&stds.x)[channel];
    }
}
int nvCropAndResizeAndNormLaunch(float* cropImages,
    ...
    float3 mean = {means[0],means[1],means[2]};
    float3 std = {stds[0],stds[1],stds[2]};
    ...
    for(int i=0;i<cpuDet.size();++i){
        ...
        if(carNum > 0){
             ...
             nvCropAndResizeAndNormKernel<<<grid,block,0,0>>>(cudaSrc[i],cropImages+(totalCars*perCarPixels),tempCudaDet+i,
                    srcWidth[i],srcHeight[i],cropW,cropH,mean,std);
             totalCars += carNum;
        }
    }
    return totalCars;
...
int nvCropAndResizeAndNormLaunch(float* cropImages,
    ...
    for(int i=0;i<cpuDet.size();++i){
        ...
        if(carNum > 0){
             ...
             nvCropAndResizeAndNormKernel<<<grid,block,0,0>>>(cudaSrc[i],cropImages+(totalCars*perCarPixels),tempCudaDet+i,tempCudaPDet+i,
                    srcWidth[i],srcHeight[i],cropW,cropH,mean,std);
             totalCars += carNum;
        }
    }
...
ITS_Vehicle_Result_Detect* initTempCudaDet(int maxDetNum){
    //std::cout << "maxDetNum is: " << maxDetNum << std::endl;
