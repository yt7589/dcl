...
public:
    ...
    ~Trt();
//	void setDataFunctor(std::function<std::vector<float >  (int)> next_batch);
     ...
     */
    bool CreateEngine(const std::string& prototxt, 
     ...
     * @return: 
     */
    bool CreateEngine(const std::string& onnxModel,
                      std::string& engineFile,
    ...
    void CopyFromHostToDeviceAsync(const std::vector<float>& input, int bindIndex);

    void CopyFromDeviceToHostAsync(std::vector<float>& output, int bindIndex);
...
private :
	template <typename T>
	bool DeserializeEngineData(const T& enginedata);
