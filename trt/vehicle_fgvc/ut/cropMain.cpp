...
std::vector<float> PreProcess(const std::vector<cv::Mat> &images)
{
    ...
    for (const auto &image : images)
    {
        ...
        std::cout<<"channels[0]: rows="<<channels[0].rows<<"; cols="<<channels[0].cols<<"; v="<<channels[0].at<uint8_t>(0, 0)<<std::endl;
        ...
        float bias = 0.0;
        channels.at(0).convertTo(imageBlue, CV_32FC1, scale, bias);
        channels.at(1).convertTo(imageGreen, CV_32FC1, scale, bias);
        channels.at(2).convertTo(imageRed, CV_32FC1, scale, bias);
    }
    return dataVec;
}
