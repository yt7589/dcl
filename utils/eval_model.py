    ...
    with torch.no_grad():
        for batch_cnt_val, data_val in enumerate(data_loader):
            ...
            for idx in range(pred_size):
                pred_brand = fgvc_id_brand_dict[top3_pos[idx][0]]
                gt_brand = fgvc_id_brand_dict[labels[idx]]
...
def predict_main(Config, model, data_loader, val_version, epoch_num, log_file):
    ...
    with torch.no_grad():
        for batch_cnt_val, data_val in enumerate(data_loader):
            ...
            for idx in range(pred_size):
                print('预测结果{0}: {1} => {2}({3});'.format(idx, data_val[4][idx], top3_pos[idx][0], top3_val[idx][0]))
            sys.exit(0)
