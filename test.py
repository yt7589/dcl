...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        for batch_cnt_val, data_val in enumerate(dataloader):
            ...
            print('eq: {0};'.format(torch.tensor(torch.eq(y_hat, labels), dtype=torch.uint8 )))
            print('total: {0};'.format(labels.shape[0]))
            print('top1 acc: {0};'.format(torch.tensor(torch.eq(y_hat, labels), dtype=torch.uint8 ) / labels.shape[0]))
