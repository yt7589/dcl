...
class CDeltaDs(object):
    ...
    def get_check_delta_ds_detls(worker_id):
        ...
        recs = []
        for row in rows:
            bmy_rst = MBmy.get_bmy_name_by_id(row['bmy_id'])
            vi_rst = MDataSource.get_vo_by_data_source_id(row['data_source_id'])
            rec = {
                'delta_ds_detl_id': row['delta_ds_detl_id'],
                'bmy_id': row['bmy_id'],
                'bmy_name': bmy_rst['bmy_name'],
                'vehicle_image_id': int(vi_rst['vehicle_image_id']),
                'state': row['state']
            }
            recs.append(rec)
        return recs